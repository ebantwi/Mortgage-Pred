...
fileslocation<- "D:/Prosper/UD Classes/Fall 2020/Multivariate stats/Project/Data/Performance_All"
...
save(Combined_Data, file="Mortgage_90day.Rda")
...
Charts<-loadWorkbook("Statistics_sanity check_90.xlsx", create = TRUE)
...
write.csv(Combined_Data,file="mortgage_data90_new.csv")


### FURTHER ANALYSIS ###
dim(Combined_Data)
#df=subset(Combined_Data,Combined_Data$LAST_STAT!=c('P'))
#dim(df)
#df$Loan_age=(as.Date(as.character(df$LAST_DTE), format="%Y/%m/%d")-
  #as.Date(as.character(df$FRST_DTE), format="%Y/%m/%d"))/60
df=subset(Combined_Data,Combined_Data$LAST_STAT!=c('S'))
df=subset(df,df$LAST_STAT!=c('R'))
df=subset(df,df$LAST_STAT!=c('F'))
df=subset(df,df$LAST_STAT!=1)
df=subset(df,df$LAST_STAT!=c('N'))
df=subset(df,df$LAST_STAT!=c('T'))
df=subset(df,df$LAST_STAT!=c('L'))
df=subset(df,df$LAST_STAT!=2)
table(df$LAST_STAT)
df_current=subset(df,df$LAST_STAT=='C')
df_prepaid=subset(df,df$LAST_STAT=='P')
df_default=subset(df,df$LAST_STAT!=c('C'))
df_default=subset(df_default,df_default$LAST_STAT!=c('P'))
write.csv(df_default,file="mortgage90_days.csv")

install.packages("splitstackshape")
library(splitstackshape)

# Current loans
table(df_current$VinYr)
df_current=subset(df_current,df_current$VinYr!=2010)
df_current=subset(df_current,df_current$VinYr!=2011)

table(df_current$VinYr)

df_current_new=stratified(df_current,c("VinYr"),17000)
write.csv(df_current_new,file="current.csv")


# Prepaid loans
table(df_prepaid$VinYr)
df_prepaid=subset(df_prepaid,df_prepaid$VinYr!=2010)
df_prepaid=subset(df_prepaid,df_prepaid$VinYr!=2011)

table(df_prepaid$VinYr)

df_prepaid_new=stratified(df_prepaid,c("VinYr"),17000)
write.csv(df_prepaid_new,file="prepaid.csv")
